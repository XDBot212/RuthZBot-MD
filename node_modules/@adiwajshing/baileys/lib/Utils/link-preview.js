 ...
 */
const getUrlInfo = async (text, opts = { thumbnailWidth: THUMBNAIL_WIDTH_PX, timeoutMs: 3000 }, logger) => {
    try {
        ...
        if (info && 'title' in info) {
            ...
            if (opts.uploadImage) {
                ...
                try {
                    ...
                    logger === null || logger === void 0 ? void 0 : logger.debug({ err: error.stack, url: previewLink }, 'error in generating thumbnail');
                }
            }
            return urlInfo;
        }
