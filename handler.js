    ...
    return new Promise(async(resolve, reject) => {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    this.sendButton(m.chat, `[❗] *Sepertinya Anda Bermain Curang, Menggunakan kalkulator*`, author, null, [['Buy Limit', '/buy limit'], ['Menu', '/menu']] , m)
                else
                    m.exp += xp
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.sendButton(m.chat, `[❗] *Limit Anda habis, Beberapa command tidak bisa diakses*`, author, null, [['Buy Limit', '/buy limit'], ['Menu', '/menu']] , m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.sendButton(m.chat, `[💬ID] Diperlukan level *${plugin.level}* untuk menggunakan perintah ini.\nLevel kamu *${_user.level}🎋*\n*[💬EN] ${plugin.level}* Level required for use this command.\nYour level is *${_user.level}🎋*`, author, null,[['Ok', 'ok']] , m)
 ...
 * @param {import('@adiwajshing/baileys').BaileysEventMap<unknown>['group-participants.update']} groupsUpdate 
 i*/ //i hope your not use this code
...
Fitur ini di *disable* !`
    ...
    if (msg) return conn.sendButton(m.chat, danied, msg, `${imgr + 'Accses Danied'}`, [['MENU', '.ruthmenu'],['OWNER', '.owner']],m)
